...
static void refill() {
  int checkFlag=0;
  if (playing==FALSE) {Serial.println("Y");return;}
  //Serial.println("filling");
  
  while(digitalRead(MP3_DREQ)){
		int nbrB= track.read(mp3DataBuffer, sizeof(mp3DataBuffer));
		if (nbrB==-1) {
			if (checkFlag==0) Serial.println("X");
			Serial.println("x");
		}
		checkFlag=1;
        if(!nbrB) { //Go out to SD card and try reading 32 new bytes of the song
			track.close(); //Close out this track
			playing=FALSE;
			Serial.println("Z"); 
			//cancel external interrupt
...
void SFEMP3Shield::transferStream(uint8_t offset, char* streamBuffer) {
	  while(!digitalRead(MP3_DREQ)){};
	  digitalWrite(MP3_XDCS, LOW); //Select Data
